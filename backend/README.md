...
Clerk owns authentication. The backend verifies Clerk session tokens
and maps them to application Users and Shops.

```text
React frontend (Clerk SDK)
      |
      | Clerk session token
      v
FastAPI
      |
      | verify Clerk token (clerk-backend-api)
      v
Clerk user_id (sub claim)
      |
      v
Application User (clerk_user_id lookup)
      |
      v
Shop membership (user.shop_id)
      |
      v
...
1. The React frontend uses `@clerk/clerk-react` to obtain a session
   token via `const token = await clerk.getToken()`.
2. The frontend sends it as `Authorization: Bearer <token>`.
3. FastAPI's `require_auth` dependency verifies the token using
   `clerk-backend-api` with `CLERK_SECRET_KEY` and `CLERK_JWT_KEY`
   (networkless RS256 verification).
4. The verified payload's `sub` claim identifies the Clerk user.
5. `get_current_user` looks up the application User by `clerk_user_id`.
6. `get_current_shop_id` returns `user.shop_id` — this is the **only**
   legitimate source of tenant context. Client-supplied `X-Shop-Id`,
   query params, and body fields are ignored.

### Environment variables

```env
CLERK_SECRET_KEY=sk_test_...          # Clerk API secret key
CLERK_JWT_KEY=-----BEGIN PUBLIC KEY-----\n...\n-----END PUBLIC KEY-----  # PEM public key for networkless verification
CLERK_AUTHORIZED_PARTIES=http://localhost:3000,https://yourapp.com
```

See [Clerk Python Backend docs](https://clerk.com/articles/how-to-add-authentication-to-a-python-backend)
for setup instructions.

### Endpoints

- `GET /auth/me` — Returns the authenticated application user and
  their shop context. No other Clerk-specific internals are exposed.

### Authorization

- `require_owner()` dependency restricts routes to OWNER role users.
- `STAFF` role has full operational access.

### Testing

Authentication tests mock `app.auth.auth._verify_token` using
`unittest.mock.patch`. No real Clerk network calls are needed.
The `conftest.py` `api_client` fixture supports dependency overrides
for test-time authentication mocking.

### Frontend integration

The frontend must use `@clerk/clerk-react`:
